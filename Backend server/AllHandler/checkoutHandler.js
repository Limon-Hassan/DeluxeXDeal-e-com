...
const {
  createSteadfastOrderSecond,
} = require('../service/steadfastServiceForAnother');
...
async function sendToSteadfast(req, res) {
  ...
  try {
    ...
    if (checkout.base && checkout.items?.[0]?.productBase) {
      return res.status(400).json({
        msg: 'This order only for steadfast second service.',
      });
    }
...
async function sendToSteadfastSecond(req, res) {
  const { id } = req.params;
  try {
    const checkout = await checkoutSchema.findById(id);
    if (!checkout) {
    ...
    if (!checkout.base || !checkout.items?.[0]?.productBase) {
      return res.status(400).json({
        msg: 'This order only for steadfast first service.',
      });
    }
    ...
    const steadfastResult = await createSteadfastOrderSecond({
      invoice: secret || checkout.uniqueOrderID,
      recipientName: checkout.name,
      recipientPhone: checkout.phone,
      recipientAddress: checkout.address,
...
async function steadfastWebhook2(req, res) {
  try {
    const authHeader = req.headers['authorization'];
    const expectedToken = `Bearer ${process.env.STEADFAST_WEBHOOK_TOKEN2}`;
...
async function AdminReadCheckout(req, res) {
  try {
    ...
    filter.$or = [{ base: { $exists: false } }, { base: '' }, { base: null }];
...
async function AdminReadCheckoutSecond(req, res) {
  try {
    const { orderStatus } = req.query;
    const filter = orderStatus ? { orderStatus } : {};
    filter.base = { $exists: true, $nin: ['', null] };
...
async function bulkUpdateOrderStatus(req, res) {
  ...
  try {
    const result = await checkoutSchema.updateMany(
      {
        _id: { $in: ids },
        $or: [{ base: { $exists: false } }, { base: '' }, { base: null }],
      },
      { $set: { orderStatus } },
    );
...
async function bulkUpdateOrderStatusSecond(req, res) {
  const { ids, orderStatus } = req.body;
...
async function updateOrderStatus(req, res) {
  ...
  try {
    const checkout = await checkoutSchema.findOneAndUpdate(
      {
        _id: id,
        $or: [{ base: { $exists: false } }, { base: '' }, { base: null }],
      },
...
async function updateOrderStatusSecond(req, res) {
  const { id } = req.params;
  const { orderStatus } = req.body;
